...
def add_item(receive_price):
    ...
    if_name_in_db = session.query(Storage.item_name).select_from(Storage).all()
    session.commit()
    if item_name in if_name_in_db:
        amount = str(input('Введите количество единиц добавляемого товара: '))
        amount_in_db = session.query(Storage.amount_left).select_from(Storage).filter(Storage.item_name == item_name)
    ...
    item = Storage(item_name=item_name, brand_id=brand, category_id=category, size_id=size, receive_datetime=receive_datetime, receive_price=receive_price, sale_price=sale_price, pottential_revenue=pottential_revenue, amount_left=final_amount, fact_marginality_percent=fact_marginality_percent)
    item_for_sell = Sold_items(item_name=item_name, brand_id=brand, category_id=category, size_id=size, receive_datetime=receive_datetime, receive_price=receive_price, sale_price=sale_price, pottential_revenue=pottential_revenue,  fact_marginality_percent=fact_marginality_percent)
    session.add(item)
